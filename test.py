...
def binary_classify():
    ...
    size = (512, 512)
    num_iter = 5
    checkpoint_path = r"files\people.pth"
...
#     model=model.to(device)
    ...
    """ Testing """
    prev_masks = init_mask(test_x[:10], size)
    save_data = []
    file = open(r"files/test_results.csv", "w")
    ...
    for iter in range(num_iter):
        ...
        for i, (x, y) in tqdm(enumerate(zip(test_x[:10], test_y[:10])), total=len(test_x[:10])):
            ## Image
...
def multiplu_classify(**weight):
    ...
    """ Directories """
    create_dir("multiplu_classify_results")
...
#     model=model.to(device)
    ...
    prev_masks = init_mask(test_x[:10], size)
    save_data = []
    file = open(r"multiplu_classify_results/test_results.csv", "w")
    file.write("Iteration,Jaccard,F1,Recall,Precision,Specificity,Accuracy,F2,Mean Time,Mean FPS\n")
...
#                 print(network_out)
    ...
    for i, (x, y) in tqdm(enumerate(zip(test_x[:10], test_y[:10])), total=len(test_x[:10])):
        ...
        cat_images = np.concatenate(tmp, axis=1)
        print(name)
        cv2.imwrite(f"multiplu_classify_results/{name}.png", cat_images)
